...
export default function Ticket({ ticketId }: { ticketId: Id<"tickets"> }) {
    ...
    const user = useQuery(api.users.getById, {
        userId: ticket?.userId as Id<"users">,
    });
    ...
    return (
        <div
            className={`bg-background rounded-xl overflow-hidden shadow-xl dark:shadow-foreground/10 border-2 ${ticket.event.is_cancelled ? "border-red-600" : "border-foreground/10"}`}
        >
            ...
            <div className="relative">
                ...
                <div
                    className={`px-6 py-4 ${imageUrl ? "absolute bottom-0 left-0 right-0" : ticket.event.is_cancelled ? "bg-red-600" : "bg-lime-500"} `}
                >
                    <h2
            ...
            <div className="p-6">
                <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center text-foreground/60">
                            <CalendarDays
                                className={`w-5 h-5 mr-3 ${ticket.event.is_cancelled ? "text-red-600" : "text-lime-600"}`}
                            />
                            <div>
                        ...
                        <div className="flex items-center text-foreground/60">
                            ...
                            />
                            <div>
                                <p className="text-sm text-foreground/50">Ticket Holder</p>
                                <p className="font-medium">{user.name}</p>
                                <p className="text-sm">{user.email}</p>
                            </div>
                        </div>

                        <div className="flex items-center text-foreground/60 break-all">
                            <IdCard
                                className={`w-5 h-5 mr-3 ${ticket.event.is_cancelled ? "text-red-600" : "text-lime-600"}`}
                            />
                            <div>
                                <p className="text-sm text-foreground/50">Ticket Holder ID</p>
                        ...
                        <div className="flex items-center text-foreground/60">
                            ...
                            />
                            <div>
                                <p className="text-sm text-foreground/50">Ticket Price</p>
                    ...
                    <div className="flex flex-col items-center justify-center border-l border-foreground/20 pl-6">
                        <div
                            className={`bg-gray-100 p-4 rounded-lg ${ticket.event.is_cancelled ? "opacity-50" : ""}`}
                        >
                            <QRCode value={ticket._id} className="w-32 h-32" />
                        </div>
                        <p className="mt-2 text-sm text-foreground/50 break-all text-center max-w-[200px] md:max-w-full">
                ...
                <div className="mt-6 pt-6 border-t border-foreground/20">
                    <h3 className="text-sm font-medium text-text-foreground/90 mb-2">
            ...
            {/* Ticket Footer */}
            <div
                className={`border border-foreground/10 px-6 py-3 flex justify-between items-center`}
            >
